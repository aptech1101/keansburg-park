...
const ZONES: Record<'PARK' | 'WATER', { name: string; logo: string }> = {
  PARK: { name: 'Zone 1', logo: '/assets/zone1.png' },
  WATER: { name: 'Zone 2', logo: '/assets/zone2.png' },
};

// Pricing helpers
const BASE_PRICE = 10;
const WEEKEND_SURCHARGE = 0.2; // $12 on Sat/Sun
const money = (n: number) => new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(n);
const isWeekend = (iso: string) => !!iso && [0, 6].includes(new Date(iso).getDay());
const unitPriceOf = (date: string) => BASE_PRICE * (isWeekend(date) ? (1 + WEEKEND_SURCHARGE) : 1);
...
const Cart: React.FC = () => {
  ...
  // Cart totals
  const totalQty = items.reduce((sum, it) => sum + Math.max(0, it.quantity || 0), 0);
  const subtotal = items.reduce((sum, it) => sum + (it.visitDate ? unitPriceOf(it.visitDate) * (it.quantity || 0) : 0), 0);
  const discount = totalQty >= 11 ? subtotal * 0.1 : 0;
  const grandTotal = subtotal - discount;

  const updateQty = (index: number, next: number) => {
    const clamped = Math.min(99, Math.max(1, Math.trunc(Number.isFinite(next as unknown as number) ? next : 1)));
    const updated = items.map((x, i) => i === index ? { ...x, quantity: clamped } : x);
    setItems(updated);
    try {
  ...
  const updateDate = (index: number, date: string) => {
    const updated = items.map((x, i) => i === index ? { ...x, visitDate: date } : x);
    setItems(updated);
    try {
      localStorage.setItem(CART_KEY, JSON.stringify(updated));
    } catch {}
    ...
    <>
      ...
      <div className="container-fluid bg-breadcrumb">
        <div className="container text-center py-5" style={{ maxWidth: 900 }}>
          <h4 className="text-white display-4 mb-4 wow fadeInDown" data-wow-delay="0.1s">Cart</h4>
          ...
          <div className="col-lg-10">
            <div className="bg-white rounded p-4 shadow-sm">
              <div className="d-flex justify-content-between align-items-center mb-3">
                ...
                <div>
                  <button className="btn btn-outline-primary me-2" onClick={() => navigate('/ticket')}>Continue shopping</button>
                  {items.length > 0 && (
              ...
              {items.length === 0 ? (
                <div className="text-center py-5">
                  <img src={imgPayment} alt="Empty cart" className="img-fluid mb-4" style={{ maxWidth: 260 }} />
                  <h5 className="mb-2">Your cart is empty</h5>
                  <p className="text-muted mb-4">Add tickets from the Ticket page to see them here.</p>
                  <Link to="/ticket" className="btn btn-primary rounded-pill px-4">Continue shopping</Link>
                </div>
              ) : (
                <>
                  {/* Table-like header */}
                  <div className="row fw-semibold text-muted px-2 mb-2">
                    ...
                    <div className="col-6 col-lg-1 text-end mt-2 mt-lg-0">Remove</div>
                  </div>
                    ...
                    <div key={(it.id || '') + idx} className="py-3">
                      <div className="row align-items-center g-3 px-2">
                        ...
                        <div className="col-12 col-lg-5 d-flex align-items-center">
                          <div className="me-3" style={{ width: 96, height: 56 }}>
                            <img
                          ...
                          <div style={{ minWidth: 220 }}>
                            <div className="fw-semibold mb-1">{ZONES[it.zoneCode].name}</div>
                            <label className="form-label small mb-1">Booking date</label>
                            <input
                              type="date"
                            ...
                            <>
                              ...
                              <div className="small mt-1">
                                {isWeekend(it.visitDate) ? (
                                  <span className="badge bg-light text-dark">Weekend $12</span>
                                ) : (
                                  <span className="badge bg-light text-dark">Weekday $10</span>
                                )}
                              </div>
                            </>
                          ) : (
                        ...
                        <div className="col-6 col-lg-2">
                          <div className="input-group" style={{ maxWidth: 160 }}>
                            <button
                              ...
                              onClick={() => updateQty(idx, (it.quantity || 1) - 1)}
                              disabled={(it.quantity || 1) <= 1 || !it.visitDate}
                            >
                              -
                            </button>
                            <input
                        ...
                        <div className="col-6 col-lg-2">
                          {it.visitDate ? (
                            <span>{money(unitPriceOf(it.visitDate) * (it.quantity || 1))}</span>
                          ) : (
                            <span>—</span>
                          )}
                        </div>
                  ...
                  <div className="row justify-content-end">
                    <div className="col-12 col-lg-5">
                      <div className="border rounded p-3">
                        ...
                        {discount > 0 ? (
                          <div className="d-flex justify-content-between mb-2">
                            <span>Group discount (10% for 11+ tickets)</span>
                            <strong className="text-success">- {money(discount)}</strong>
                          </div>
                        ) : (
                          <div className="d-flex justify-content-between mb-2">
                            <span>
                              Group discount (10% for 11+ tickets)
                              {totalQty < 11 && (
                                <small className="text-muted ms-2">Add {Math.max(0, 11 - totalQty)} more to get 10% off</small>
                              )}
                            </span>
                            <strong>—</strong>
                          </div>
                        ...
                        <button className="btn btn-primary rounded-pill mt-3 w-100" disabled={!canProceed} onClick={() => {
                  ...
                  <div className="toast-container position-fixed bottom-0 end-0 p-3" style={{ zIndex: 1080 }}>
                    <div className={`toast ${toastVisible ? 'show' : 'hide'}`} role="status" aria-live="polite" aria-atomic="true">
                      <div className={`toast-header ${toastVariant === 'success' ? 'bg-success text-white' : 'bg-danger text-white'}`}>
                        <strong className="me-auto">{toastVariant === 'success' ? 'Success' : 'Notice'}</strong>
