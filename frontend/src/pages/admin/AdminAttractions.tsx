...
const UPLOAD_API_URL = `${apiConfig.baseURL}/admin/upload`; // 1 file upload chung

// Raw shape from backend (features may be string/JSON/string[])
...
// Normalized shape used in UI
interface AttractionView {
  id: number;
  zone_id: number;
  name: string;
  description: string;
...
type AttractionForm = Omit<AttractionView, "id" | "zone_name"> & {
...
const AdminAttractions: React.FC = () => {
  const [attractions, setAttractions] = useState<AttractionView[]>([]);
  ...
  const [selectedZone, setSelectedZone] = useState<string>('all'); // 'all', 'amusement', 'water'
  ...
  const fetchAttractions = async () => {
    try {
      ...
      const raw: AttractionRaw[] = res.data?.data || [];
      const normalized = raw.map((a) => ({
        ...a,
        features: toFeaturesArray(a.features as any),
      })) as AttractionView[];
  ...
  const [selectedItem, setSelectedItem] = useState<AttractionView | null>(null);
  ...
  const uploadImage = async (): Promise<string> => {
    ...
    formData.append("type", "attraction"); // để backend phân loại thư mục
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingId) {
        ...
        setMessage({ type: 'success', text: 'Attraction added successfully!' });
      }
      ...
      setMessage({ type: 'error', text: 'Error saving attraction. Please try again.' });
      setTimeout(() => setMessage({ type: null, text: '' }), 5000);
    }
  };

  const handleEdit = (attraction: AttractionView) => {
    const featuresArray = toFeaturesArray(attraction.features as any);
    setForm({
      ...
      featuresInput: featuresArray.join(", "), // Populate featuresInput
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: 'Error deleting attraction. Please try again.' });
      setTimeout(() => setMessage({ type: null, text: '' }), 5000);
    }
  };

  // Filter attractions by zone
  const filteredAttractions = attractions.filter(attraction => {
    if (selectedZone === 'all') return true;
    const zoneName = (attraction.zone_name || '').toLowerCase();
  ...
  const paginatedAttractions = filteredAttractions.slice(startIndex, startIndex + itemsPerPage);
  ...
  return (
    <div className="container mt-4 admin-attractions admin-table admin-form admin-modal admin-pagination">
      <style>{`
        .admin-attractions .form-control,
        .admin-attractions .form-select,
        .admin-attractions textarea {
          border: 2px solid #e9ecef;
          box-shadow: none;
          transition: border-color .2s ease, box-shadow .2s ease;
        }
        .admin-attractions .form-control:focus,
        .admin-attractions .form-select:focus,
        .admin-attractions textarea:focus {
          border-color: #3CBEEE;
          box-shadow: 0 0 0 0.2rem rgba(60,190,238,.15);
        }
        /* Ngăn layout shift khi hover highlight từ theme khác */
        .admin-attractions .card,
        .admin-attractions .card:hover,
        .admin-attractions table,
        .admin-attractions table:hover,
        .admin-attractions .table,
        .admin-attractions .table:hover {
        ...
        .admin-attractions .table thead th,
        .admin-attractions .table tbody td {
          transition: none !important;
        }
      `}</style>

      <div className="d-flex justify-content-between align-items-center mb-3">
        <h2 className="mb-0">Attractions Management</h2>
        <button
          className={`btn ${showForm ? "btn-secondary" : "btn-primary"}`}
          onClick={() => setShowForm(!showForm)}
        >
          {showForm ? "Close form" : "+ Add attraction"}
        </button>
      </div>
      ...
      <div ref={formRef} className="card p-4 mb-4 shadow-sm">
        <div className="row g-3">
          ...
          <div className="col-md-4">
            <label className="form-label fw-bold">Attraction name</label>
            <input
              type="text"
          ...
          <div className="col-12 col-md-8">
            ...
            <input
              type="text"
              className="form-control"
              placeholder="e.g. Family-friendly, Thrilling, Indoor"
      ...
      <div className="card shadow-sm">
          <div className="table-responsive">
            <table className="table table-bordered table-hover">
              ...
              <tbody>
                {paginatedAttractions.map((a) => (
                  <tr key={a.id}>
                    <td>{a.id}</td>
                    <td>{a.zone_name || a.zone_id}</td>
                    <td>{a.name}</td>
                    <td>
                      {a.description && a.description.length > 20
                    ...
                    <td>
                      {(() => {
                        if (!a.features || !Array.isArray(a.features)) return "";
                        const validFeatures = a.features.filter(f => f && typeof f === 'string' && f.trim().length > 0);
                    ...
                    <td>
                      {a.details && a.details.length > 20
                        ? a.details.substring(0, 20) + "..."
                        : a.details}
                    </td>
                    <td>
                      {a.image_url && (
                        <img
                          src={toBackendUrl(a.image_url)}
                          alt={a.name}
                          style={{ width: "80px" }}
      ...
      {/* Modal hiển thị chi tiết Attraction */}
      <Modal
        show={showModal}
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            {selectedItem ? selectedItem.name : "Attraction Detail"}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedItem && (
            <div style={{ whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
              ...
              {selectedItem.image_url && (
                <img
                  src={toBackendUrl(selectedItem.image_url)}
