...
// ⚡ Import FeedbackContext để cập nhật badge
...
const AdminFeedback: React.FC = () => {
  ...
  // ⚡ Lấy hàm refreshPendingCount từ FeedbackContext
  const { refreshPendingCount } = useFeedback();
  ...
  // ⚡ Cập nhật status feedback và badge
  const updateFeedbackStatus = async (id: number, status: 'approved' | 'rejected') => {
    try {
      ...
      if (res.data.status === "success") {
        ...
        refreshPendingCount();     // ⚡ refresh badge ở sidebar
      } else {
  ...
  // ⚡ Xóa feedback và cập nhật badge
  const deleteFeedback = async (id: number) => {
    ...
    try {
      ...
      if (res.data.status === "success") {
        fetchFeedbacks();          // refresh table
        refreshPendingCount();     // ⚡ refresh badge
      } else {
  ...
  return (
    <div className="container-fluid admin-feedback admin-table admin-form admin-pagination">
      ...
      {loading ? (
        ...
        <>
          <div className="table-responsive admin-feedback">
