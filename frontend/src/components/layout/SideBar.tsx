...
const Sidebar: React.FC<SidebarProps> = ({ onClose }) => {
  ...
  const menuItems = [
    ...
    { to: "/admin/contact", icon: "fas fa-envelope", label: "Contact" },
    { to: "/admin/feedback", icon: "fas fa-comments", label: "Feedback", badge: pendingCount }
  ];
        ...
        {menuItems.map((item) => (
          <li key={item.to} className="nav-item mb-1">
            ...
            >
              ...
              <span className="d-md-none text-truncate" title={item.label} style={{ maxWidth: "140px", fontSize: "14px" }}>
                {item.label}
              </span>
              {item.badge ? (
