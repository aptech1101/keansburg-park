    ...
    switch ($method) {
        case 'GET':
            if (isset($_GET['id'])) {
                ...
                // Attractions
                $stmt = $pdo->prepare("SELECT * FROM attractions WHERE zone_id = ?");
                $stmt->execute([$zoneId]);
                $zone['attractions'] = $stmt->fetchAll(PDO::FETCH_ASSOC);

                // Restaurants
                $stmt = $pdo->prepare("SELECT * FROM restaurants WHERE zone_id = ?");
                $stmt->execute([$zoneId]);
                $zone['restaurants'] = $stmt->fetchAll(PDO::FETCH_ASSOC);

                // Tickets
                $stmt = $pdo->prepare("SELECT * FROM tickets WHERE zone_id = ?");
                $stmt->execute([$zoneId]);
