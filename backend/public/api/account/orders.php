...
try {
    ...
    if ($startDate) { $whereConditions[] = 'DATE(b.visit_date) >= ?'; $params[] = $startDate; }
    if ($endDate) { $whereConditions[] = 'DATE(b.visit_date) <= ?'; $params[] = $endDate; }
    if ($status) { $whereConditions[] = 'b.status = ?'; $params[] = $status; }
